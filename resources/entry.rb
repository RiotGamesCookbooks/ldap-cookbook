...
property :distinguished_name, String, name_attribute: true
property :attributes, Hash
property :append_attributes, Hash
property :seed_attributes, Hash
property :prune, [ Array, Hash ], default: Array.new
...
action :create do
  ...
  converge_if_changed("Entry #{@new_resource.distinguished_name}") do
...
action :delete do
  ...
  if @current_resource
    converge_if_changed("Removing #{@current_resource.distinguished_name}") do
      ldap = Chef::Ldap.new
      @connectinfo = load_connection_info
...
action_class.class_eval do
  ...
  def load_current_resource
    ...
    if entry
      @current_resource = ::Chef::Provider::LdapEntry.new entry.dn
